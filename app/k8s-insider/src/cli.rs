...
impl Default for CreateTunnelArgs {
    fn default() -> Self {
        Self {
            network: DEFAULT_NETWORK_NAME.into(),
...
pub struct GetConfArgs {
    /// Name of the tunnel to generate configuration for (can be omitted if there's only one tunnel in the config)
    #[arg()]
    pub name: Option<String>,
...
pub enum ConfigSubcommands {
    ...
    #[command()]
    AddTunnel(ConfigAddTunnelArgs),
    /// List tunnels defined in the config
    #[command()]
    ListTunnels(ConfigListTunnelsArgs),
    /// Remove a tunnel from the configuration (but not from the cluster)
    #[command()]
    RemoveTunnel(ConfigRemoveTunnelArgs),
}
